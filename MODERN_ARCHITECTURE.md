...
<template>
  <TrackList v-if="!isLoading" :show-selection="true" @song-play="handlePlay" />
</template>
```
...
<template>
  ...
  <VirtualizedList :items="songs" :item-height="64" :container-height="600">
    <template #item="{ item: song }">
      <SongItem :song="song" />
    </template>
  </VirtualizedList>
