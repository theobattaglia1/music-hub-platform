...
// Computed
// Custom mode: items have callback functions (from views) rather than being data objects
const isCustomMode = computed(() =>
  items.value.some(it => typeof it?.action === 'function' || it?.divider === true)
)
...
const visibleActions = computed(() => {
  if (isCustomMode.value) {
    return items.value.map(item => {
      ...
      return {
        label: item.label,
        action: item.action, // keep as function reference
        icon: item.icon || null,
        destructive: item.danger || item.destructive || false,
        submenu: false
      }
    })
  }
...
const handleAction = (action) => {
  ...
  // Custom callback mode: action.action is a function
...
.context-menu {
  ...
  padding: 4px;
  box-shadow: var(--shadow-overlay), 0 0 0 1px var(--color-border);
  color: var(--color-text);
  font-size: 13px;
  overflow: hidden;
  outline: none;
  /* override glassmorphic to be fully opaque */
