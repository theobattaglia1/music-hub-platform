          ...
          <div v-if="flatItems.length" class="search-results" ref="resultsEl">
            <template v-for="(item, idx) in flatItems" :key="item._id">
              ...
              >
                ...
                <svg class="result-caret" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
const PAGES = [
  ...
  { num: '05', label: 'Projects',           path: '/projects',   meta: 'Projects' },
  ...
  { num: '10', label: 'Media Library',      path: '/media',      meta: 'Creative' },
  { num: '11', label: 'Moodboards',         path: '/moodboards', meta: 'Creative' },
  { num: '20', label: 'Files',              path: '/files',      meta: 'Documents' },
  { num: '35', label: 'Analytics',          path: '/reports',    meta: 'Reports' },
...
const flatItems = computed(() => {
  ...
  if (!q) {
    ...
    PAGES.slice(0, 8).forEach((p, i) =>
      result.push({ ...p, _id: `page-${i}`, _type: 'page' })
    )
    return result
  }
  ...
  // Artists
  const matchedArtists = props.artists.filter(a =>
...
const selectableIdxs = computed(() =>
  flatItems.value
    .map((item, i) => item._type !== 'header' ? i : -1)
    .filter(i => i >= 0)
)
...
const move = (dir) => {
  ...
  if (next !== undefined) {
    ...
    nextTick(() => {
      resultsEl.value
        ?.querySelector('.result-row.is-active')
