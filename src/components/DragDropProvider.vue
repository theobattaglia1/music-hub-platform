...
<template>
  <div class="drag-drop-provider">
    ...
    <!-- Drop Zones Overlay -->
    <teleport to="body">
      <div v-if="dragState.isDragging && dropZones.length > 0" class="drop-zones-overlay">
        <div
...
.drag-preview-content {
  ...
  border: 1px solid rgba(232, 90, 25, 0.28);
}
...
:global(.drop-target-highlighted) {
  outline: 2px dashed rgba(232, 90, 25, 0.4);
  outline-offset: 2px;
  transition: outline 0.2s ease;
}

:global(.drop-zone-hover) {
  background: rgba(232, 90, 25, 0.1) !important;
