...
export const useLibraryStore = defineStore("library", () => {
  ...
  const audioFiles = computed(() =>
    (media.value || []).filter((item) => item.media_type === "audio"),
  );
  const videoFiles = computed(() =>
    (media.value || []).filter((item) => item.media_type === "video"),
  );
  const imageFiles = computed(() =>
    (media.value || []).filter((item) => item.media_type === "image"),
  );
  const documentFiles = computed(() =>
    (media.value || []).filter((item) => item.media_type === "document"),
  );
