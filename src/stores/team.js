...
export const useTeamStore = defineStore("team", () => {
  ...
  const acceptInvitationMutation = useMutation({
    mutationFn: async ({ artistId, userId }) => {
      const { data, error } = await apiService.update(
        "artist_team",
        null,
        {
  ...
  const hasPermission = (artistId, permission) => {
    const member = (teamMembers.value || []).find(
      (member) => member.artist_id === artistId && member.user_id === currentUserId.value,
    );
  ...
  return {
    ...
    isBulkUpdating: computed(
      () => bulkUpdateRolesMutation.isPending.value || bulkRemoveMembersMutation.isPending.value,
    ),
  };
});
