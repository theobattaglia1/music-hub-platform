...
const LOCAL_ARTISTS = ref([
  ...
  },
  {
    ...
    team_members: [{ id: 3, name: "Mike Engineer", role: "Engineer", avatar_url: null }],
  },
  {
    id: "3",
    name: "Billie Eilish",
...
export const useDashboardStore = defineStore("dashboard", () => {
  ...
  const recentArtists = computed(() => {
    return artists.value
      .sort(
        (a, b) =>
          new Date(b.last_activity || b.updated_at) - new Date(a.last_activity || a.updated_at),
      )
      .slice(0, 5);
  });
  ...
  const loadDashboardData = async () => {
    try {
      ...
      await Promise.all([loadArtists(), loadRecentActivity(), loadStats()]);
    } catch (error) {
  ...
  const createArtist = async (artistData, avatarFile = null) => {
    try {
      ...
      const newArtist = {
        ...
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
        team_members: [{ id: "demo-user", name: "Demo User", role: "owner", avatar_url: null }],
      };
