...
export function useCreateArtist() {
  ...
  return useMutation({
    mutationFn: ({ artistData, avatarFile }) => artistService.createArtist(artistData, avatarFile),
    onSuccess: (newArtist) => {
      // Invalidate and refetch artists list
      invalidateQueries.artists();

      // Add to cache
      queryClient.setQueryData(queryKeys.artists.detail(newArtist.id), newArtist);
    },
    onError: (error) => {
  ...
  return useMutation({
    ...
    onError: (err, variables, context) => {
      ...
      if (context?.previousArtist) {
        queryClient.setQueryData(queryKeys.artists.detail(context.id), context.previousArtist);
      }
    },
    onSuccess: (updatedArtist, { id }) => {
      // Update cache with server response
      queryClient.setQueryData(queryKeys.artists.detail(id), updatedArtist);
      invalidateQueries.artists();
    },
  });
}
